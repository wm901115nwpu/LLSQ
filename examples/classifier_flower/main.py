...
import models.flower102 as flower102_models
from examples import *

model_names = sorted(name for name in flower102_models.__dict__
                     if name.islower() and not name.startswith("__")
                     and callable(flower102_models.__dict__[name]))
...
def main():
    ...
    parser.add_argument('-a', '--arch', metavar='ARCH', default='flower102_vggsmall',
                        choices=model_names,
                        help='model architecture: ' +
                             ' | '.join(model_names) +
                             ' (default: flower102_vggsmall)')
...
def main_worker(gpu, args):
    ...
    try:
        model = flower102_models.__dict__[args.arch](pretrained=args.pretrained, nbits_w=args.qw, nbits_a=args.qa)
    except KeyError:
    ...
    print('model:\n=========\n{}\n=========='.format(model))
    if args.gen_map:
        main_gen_key_map(args, model, flower102_models)
        return
    ...
    train_loader, val_loader = df.product_train_val_loader(df.flower102)
