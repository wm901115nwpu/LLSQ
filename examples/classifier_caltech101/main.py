...
import models.caltech101 as caltech101_models
from examples import *

model_names = sorted(name for name in caltech101_models.__dict__
                     if name.islower() and not name.startswith("__")
                     and callable(caltech101_models.__dict__[name]))
best_acc1 = 0


def main():
    parser = get_base_parser()
    parser.add_argument('-a', '--arch', metavar='ARCH', default='caltech101_vggsmall',
                        choices=model_names,
                        help='model architecture: ' +
                             ' | '.join(model_names) +
                             ' (default: caltech101_vggsmall)')
...
def main_worker(gpu, args):
    ...
    try:
        model = caltech101_models.__dict__[args.arch](pretrained=args.pretrained, nbits_w=args.qw, nbits_a=args.qa)
    except KeyError:
    ...
    print('model:\n=========\n{}\n=========='.format(model))
    if args.gen_map:
        main_gen_key_map(args, model, caltech101_models)
        return
    ...
    train_loader, val_loader = df.product_train_val_loader(df.caltech101)
